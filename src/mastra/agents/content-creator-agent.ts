...
2. Before writing the posts, shorten the article URL once using the connected Dub link-shortener tools (create a short link for the article URL), then use that short link - not the raw article URL - in each post's text where it fits naturally, so readers can click through. If shortening fails or no Dub tools are connected, fall back to the raw article URL rather than blocking.
