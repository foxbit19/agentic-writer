...
Writes **short, platform-native teaser posts** from the strategy (not article recaps) and a creative brief for the hero image (handed off to the Graphic Designer rather than generating it itself); uses the article URL exactly as given (the workflow has already shortened it via Dub when `DUB_API_KEY` is set). The social workflow saves output to disk for manual review and publishing.
