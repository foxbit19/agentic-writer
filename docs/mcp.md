...
| Tool | Input | Notes |
|------|-------|--------|
| `start_article_workflow` | `{ notes, authorDraft? }` | `notes` = operating instructions; optional `authorDraft` = prose to develop. Blocks until first human-approval suspend (after research and up to three Writer–Editor polish passes) or completion. Can take several minutes. Suspend payload includes `editorReady` and `editorPassCount`. |
...
| `reject_draft` | `{ articleId, notes }` | Resume with `approved: false` and revision guidance; Writer revises once (Editor skipped). Waits for next suspend. |
...
1. `start_article_workflow` with notes (and optional authorDraft) → returns `suspended` + draft, editor review, `editorReady`, and `editorPassCount` in `suspendPayload`.
2. Inspect with `get_article_drafts` / `get_article_status` if needed.
3. `approve_draft` or `reject_draft` (reject runs Writer once without Editor, then loops until approve).
